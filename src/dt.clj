...
(defn load-photos []
  (let [photo-records-report (bru/load-latest-report)
        ...
        photo-records  (filter #(.exists (clojure.java.io/as-file (first %))) photo-records-no-assets)]
    (println "Loaded" (count photo-records) "photo records (no assets/no delete files)")
...
(comment
  ...
  ;
  "this is here to bring the paren down")
