...
(defn load-photos []
  (let [photo-records-report (bru/load-latest-report)
        ...
        _ (println "Loaded"  (count photo-records-no-del) "with no missing (deleted) files")
        photo-records (filter
                        #(not= "smaller" (.getName (.getParentFile (clojure.java.io/as-file (first %)))))
                         photo-records-no-del)]
...
(defn find-with-dups-from-dir
  ([dups dir ] (find-with-dups-from-dir dups dir #{}))
  ([dups dir dups-found]
   (if (empty? dups) dups-found
                  (let [fdup (first dups)
                        ffile (first fdup)
                        sfile (second fdup)]
                    (if (.startsWith ffile dir)
                      (recur (rest dups) dir (conj dups-found ffile))
                      (if (.startsWith sfile dir)
                        (recur (rest dups) dir (conj dups-found sfile))
                        (recur (rest dups) dir dups-found)))))))
...
(defn short-disp-dups [dups]
  (doseq [x dups] (printf "%.30s %-50s %s%n" (get x 2) (shorten-path (first x)) (shorten-path (second x)))))


(defn delete-files [coll]
  (doseq [f coll] (try (clojure.java.io/delete-file f) (println "deleted" f) (catch Exception e (println "bah on" f)))))

(defn show-dups [dups dir]
  (filter #(or (.startsWith (second %) dir) (.startsWith (first  %) dir)) dups))
